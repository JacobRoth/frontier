...
--lwarrior kgrounddronestructure lengineer kairdronestructure laadrone
...
function canUnitBuildThis (parentName, childName)
	...
	if (parentName == "lengineer" and childName == "kdroneminingtower") then return true end
	...
	for _,t in pairs(myTeam) do
		...
		if (frame % 450 ==0) then --every 15 secs
			...
			for i=1,unitsWeMake do
				unitWeWillMake = unweighted_choice( {"lwarrior", "laadrone", "lwarrior", "laadrone","lwarrior", "laadrone", "lwarrior", "laadrone", "kdiairdrone", "kdiaridrone", "ktriairdrone", "ktriairdrone", "ktriairdrone" } )
...
function gadget:UnitFinished(unitID, unitDefID, teamID)	
	...
	if (isTeamCBM (teamID)) then
		...
		if ((unitName (unitID) ~= "lengineer") or (unitName (unitID) ~= "ktridroneroller")) then
